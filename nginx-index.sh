...
INFLUXDBSHELL=$(kubectl exec -n influxdb ${POD} -- influx --version | cut -d " " -f 4 | sed 's/.$//')
INFLUXDB=$(kubectl exec -n influxdb ${POD} -- influxd version | cut -d " " -f 2)
...
FLUENTBIT=$(kubectl exec ${POD} -n fluentbit -- /fluent-bit/bin/fluent-bit --version | cut -d " " -f 3)

POD=$(kubectl get pods -n ingress-nginx | grep "^ingress-nginx-controller-" | cut -d " " -f 1)
INGRESSNGINXNGINX=$(kubectl exec -n ingress-nginx ${POD} -- nginx -v 2>&1 | cut -d "/" -f 2)
...
NGINX=$(kubectl exec ${POD} -n nginx -c nginx -- /opt/bitnami/nginx/sbin/nginx -v 2>&1 | cut -d "/" -f 2 )
...
GRAFANACLOUDAGENT=$(kubectl exec ${POD} -n grafana-cloud -- /bin/agent --version 2>&1 | grep "^agent" | cut -d " " -f 3)

MYAPP=$(curl -s http://localhost:${HTTPPORT}/service/info | jq -r .serverVersion)
...
echo "  <style>"
echo "    .disable { color: grey; }"
echo "    .disable a:link { color: grey; }"
echo "    .disable a:visited { color: grey; }"
echo "    .disable a:hover { color: grey; }"
echo "    .disable a:active { color: grey; }"
echo "  </style>"
echo "</head>"
echo "<body>"
...
[ -z "${DASHBOARDHELM}" ] && echo "  <div class=\"disable\">"
  echo "  <li><a href=\"http://localhost:${HTTPPORT}/dashboard/#/workloads?namespace=_all\"><b>Kubernetes Dashboard</b></a></li>"
[ -z "${DASHBOARDHELM}" ] && echo "  </div>"
echo "  <li>NodeJS App ${APP} APIs: <a href=\"http://localhost:${HTTPPORT}/service/info\"><b>info</b></a>, "
echo "  <a href=\"http://localhost:${HTTPPORT}/service/random\"><b>random</b></a>, "
echo "  <a href=\"http://localhost:${HTTPPORT}/service/metrics\"><b>metric</b></a></li>"
...
echo " <a href=\"http://localhost:${HTTPPORT}/alert\"><b>Alertmanager</b></a></li>"
...
[ ! "${GRAFANA_CLOUD_ENABLE}" == "yes" ] && echo "  <div class=\"disable\">"
  echo "  <li>Grafana Cloud: <a href=\"https://grafana.com/orgs/${GRAFANA_CLOUD_ORG}\"><b>Portal</b></a>, "
...
echo "See also <a href=\"https://klaushofrichter.medium.com/\">Medium.com</a> for a live listing."
echo "<ul>"
